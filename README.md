...
| OTX            | otx.alienvault.com      | Threat intelligence data |
| AbuseIPDB      | abuseipdb.com           | IP blacklist + Geo data  |
...
Why GitHub Actions for scheduling? Free, reliable, no extra server needed. 2000 minutes/month on free tier — running every 15 min uses ~1400 minutes/month.

Why simulated data? Lets the whole team develop and demo the dashboard without needing API keys. The simulator generates statistically realistic attack distributions that exercise all chart types properly.
